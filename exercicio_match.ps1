...
$metodo = Read-Host "Introduza o método (ex: GET, POST)"
$conteudo = Read-Host "Introduza o conteúdo (basta dar Enter para deixar vazio)"
$requisicao = @{ "metodo" = $metodo; "conteudo" = $conteudo }

switch ($requisicao.metodo.ToUpper().Trim()) {
    'GET'  { "Requisição GET recebida" }
    'POST' {
        if ([string]::IsNullOrWhiteSpace($requisicao.conteudo)) { "Requisição POST sem dados" }
        else { "Requisição POST com dados válidos" }
    }
...
$jogador1 = Read-Host "Jogador 1: pedra, papel ou tesoura?"
$jogador2 = Read-Host "Jogador 2: pedra, papel ou tesoura?"

$j1 = $jogador1.ToLower().Trim()
$j2 = $jogador2.ToLower().Trim()

if ($j1 -eq $j2) { 
    "Empate" 
} else {
    switch ($j1) {
        'pedra'   { if ($j2 -eq 'tesoura') { "Jogador 1 venceu" } else { "Jogador 2 venceu" }; break }
        'papel'   { if ($j2 -eq 'pedra')   { "Jogador 1 venceu" } else { "Jogador 2 venceu" }; break }
        'tesoura' { if ($j2 -eq 'papel')   { "Jogador 1 venceu" } else { "Jogador 2 venceu" }; break }
        default   { "Alguém jogou algo inválido!" }
    }
}